...
class SimilarityBasedModel(object):
    ...
    def fit(self, interactions):
        ...
        self._interactions_ones = coo_matrix((np.ones(len(interactions.ratings)), (interactions.user_ids, interactions.item_ids)),
                      shape=(interactions.num_users, interactions.num_items)).tocsr()

    def predict(self, user_ids, item_ids=None):
        return pp.minmax_scale((self._interactions_ones[user_ids] * self._sim)[0]) * 5


    def _predict_k(self, user_ids, item_ids=None, k=50):

        self._check_input(user_ids, item_ids, allow_items_none=True)
        user = self._interactions_ones[user_ids]
        ...
        return np.sum(np.array(sim_filtered_k) * data_mat, axis=1)
